#a code for gmm creation from mfcc with correct methods but cleaner here insted aof e^x we take log(e^x)=x so chages made accordingly
import numpy as np
import math
from scipy import linalg
from mpmath import *
def multivar(array2d,mean,sigma): #this functions clacutates the multivariate function of the gmm ie the fxx value
	pi =np.log( (2*math.pi)**6.5)
	x=array2d
	xu=x-mean
	...
	si=np.log((np.linalg.det(sigma))**0.5)
	#print si
...
def resterm(w1,w2,fxxn,fxxd,numrows): #here fxxn is the neumerator fxx and the fxxd is the denomination fxx in eqn rn(1)=w1*fxx1(xn,u1,e1)/(w1*fxx1(xn,u1,e1)+w2*fxx2(xn,u2,e2))
	#a=[np.exp(x) * w1 for x in fxxn] #this does w[0]*fxxn 
	#b=[np.exp(x) * w2 for x in fxxd] #this does w[1]*fxxd
	w=w2/w1
	r=np.zeros(numrows)
	val=0
	for i in range (0,numrows):
		val=np.exp(fxxd[i]-fxxn[i])
		r[i]=1/(1+w*val)
	 #depends on the total no of gausians since we have 2 gausians here so 2 fxx
	#for i in range (0,numrows):
		#print a[i]+b[i]
	#	r[i]=a[i]/(a[i]+b[i])
	#print fxxn[0]	
...
fxx1=np.zeros(numrows) #depends on the total no of gausians since we have 2 gausians here so 2 fxx
fxx2=np.zeros(numrows) #depends on the total no of gausians since we have 2 gausians here so 2 fxx
...
while(ck==1):
	for i in range (0,numrows):
		fxx1[i]=multivar(array2d[i],mean1,sigma1)
		fxx2[i]=multivar(array2d[i],mean2,sigma2)
	...
	#						ck=0 #to break while loop
	score = 0.0
	for n in range (0,numrows):
		score += np.log(w1*r1[n])
		score += np.log(w2*r2[n])
	print w1,w2	
	ww1=w1
	ww2=w2
	mean1=u1
